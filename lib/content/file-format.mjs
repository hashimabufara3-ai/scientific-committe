...
/* Upload cap (10 MB), shared by the client hint and the server validation. */
...
/* The ONLY object paths the application ever issues for a finalized resource:
     summaries/<uuid-v4>.<ext>
     exams/<uuid-v4>.<ext>
   where <ext> is one of the allowlisted extensions above. This is the single
   source of truth shared by the SQL RPC guard (an equivalent regex lives in the
   migration), the download/access boundaries, and the unit tests.

   The pattern is intentionally strict and anchored: it rejects NULL-like
   non-strings, empty strings, leading slashes, quarantine/* staging objects,
   other prefixes, `..`/percent-encoded traversal, extra path segments, query
   strings, whitespace padding, non-v4 UUIDs, and non-allowlisted extensions.
   Callers must REJECT a non-match — never normalize or repair it. */
export const CANONICAL_RESOURCE_PATH_PATTERN =
  /^(summaries|exams)\/[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.(pdf|jpg|png|webp|gif)$/i;

/* Validate a DB/storage object path against the canonical contract.
   `kind` optionally narrows the expected prefix ("summary" -> summaries/,
   "exam" -> exams/). Returns true only for an exact, canonical match. */
export function isCanonicalResourcePath(path, kind) {
  if (typeof path !== "string" || path.length === 0) return false;
  if (!CANONICAL_RESOURCE_PATH_PATTERN.test(path)) return false;
  if (kind === "summary") return path.startsWith("summaries/");
  if (kind === "exam") return path.startsWith("exams/");
  return true;
}